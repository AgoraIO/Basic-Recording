// Copyright (c) 2018-2019 Agora.io, Inc.
// cloudrecording@agora.io
...
//     CM_LOG(ERROR, "short parameters should be alphabetic!");
//     return false;
//   }
//
//   if (short_args_.count(short_arg) > 0) {
//     CM_LOG(ERROR, "short parameter[%c] has been occupied!", short_arg);
//     return false;
//   }
//
...
bool opt_parser::insert_long_opt(internal_opt opt, const char *long_opt) {
  ...
  if (opt.help == NULL || !strlen(opt.help)) opt.help = "NA";
...
bool opt_parser::add_long_opt(const char *long_opt, bool *store,
                              const char *help,
                              opt_parser::opt_type optParamType) {
  internal_opt arg;
  arg.type = kBool;
  arg.bool_ptr = store;
  arg.help = help;
...
bool opt_parser::add_long_opt(const char *long_opt, std::string *store,
                              const char *help,
                              opt_parser::opt_type optParamType) {
  internal_opt arg;
  arg.type = kString;
...
bool opt_parser::parse_mac_addr(const char *arg, mac_addr *addr) {
  ...
  if (sscanf(arg, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &b[0], &b[1], &b[2], &b[3],
             &b[4], &b[5]) != 6) {
...
bool opt_parser::fill_arg(const char *opt_name, const internal_opt &opt,
  ...
  switch (opt.type) {
    case kBool: {
      unsigned long n = strtoul(opt_arg, &end_ptr, 10);  // NOLINT(runtime/int)
      if (*end_ptr != '\0') {
    ...
    case kInt32: {
      long n = strtol(opt_arg, &end_ptr, 10);  // NOLINT(runtime/int)
      if (*end_ptr != '\0') {
        CM_LOG(ERROR, "Invalid integer argument: %s", opt_arg);
        return false;
      }
    ...
    case kInt64: {
      long long n = strtoll(opt_arg, &end_ptr, 10);  // NOLINT(runtime/int)
      if (*end_ptr != '\0') {
        CM_LOG(ERROR, "Invalid integer argument: %s", opt_arg);
        return false;
      }
    ...
    case kIPv4: {
      if (!parse_ipv4(opt_arg, opt.ipv4_ptr)) return false;
      break;
    }
    case kMacAddr: {
      if (!parse_mac_addr(opt_arg, opt.addr_ptr)) return false;
      break;
    }
    default:
      assert(false);
...
void opt_parser::save_to_exopts() const {
  typedef std::unordered_map<const char *, internal_opt>::const_iterator It;
...
bool opt_parser::parse_opts(int argc, char *const argv[]) {
  ...
  //
  //  std::unordered_map<char, internal_opt>::const_iterator it;
  ...
  std::unordered_map<const char *, internal_opt>::const_iterator f;
  for (f = long_opts_.begin(); f != long_opts_.end(); ++f) {
    option arg = {f->first, f->second.optType == opt_argu ? optional_argument
                                                          : required_argument,
                  0, 0};
  ...
  while ((ret = getopt_long_only(argc, argv, "", &long_opt[0], &index)) == 0 ||
         (ret > 0 && ret != '?')) {
...
void opt_parser::print_usage(const char *exec_file, std::ostream &sout) const {
  sout << "Usage: \n  " << exec_file << " ";
